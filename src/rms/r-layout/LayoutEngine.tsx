...
export function LayoutEngine() {
    ...
    const currentModule = config?.modules.find(m =>
        m.path === "/" ? location.pathname === "/" : location.pathname.startsWith(m.path)
    );
    const shell = (currentModule && config?.shells[currentModule.shellId]) || config?.shells["default"];
    ...
    const drawerContent = (
        <Box sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
            ...
            >
                ...
                {shell?.sidebarCollapsible !== false && (
                    <IconButton
                        size="small"
